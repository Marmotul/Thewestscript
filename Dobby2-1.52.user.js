...
(function() {
	...
	const STATE_WAITING_CONSUMABLE = "Waiting for a consumable cooldown";
	const STATE_DEPOSITING_MONEY = "Depositing money";
	const STATE_SLEEPING = "Sleeping";
	const SILVER_RESET_HOUR = 2;
	...
	const Dobby = {
		...
		consumableUsed:[],
		addedJobs:[],
		jobFilter:{filterOnlySilver:false,filterNoSilver:false,filterCenterJobs:false,filterJob:"", sortDirection: "", sortField: ""},
	...
	Dobby.isInMapCenter = function(x, y) {
		if(x >= 19968 && x <= 26623){
			if(y >=6912 && y <= 13567){
				return true
			}
	...
	Dobby.redefineFunctions = function () {
		TaskQueue.sendCancel = function () {
			return Ajax.remoteCall('task', 'cancel', {
				'tasks': TaskQueue.toCancel
			}, function (data) {
				TaskQueue.toCancel = {};
				$.each(data['tasks'],
					function (queuePos, queueData) {
						var task = TaskQueue.queue[queuePos];
						if (task !== undefined) {
							if (queueData['extra']) task.cancelled(queueData['extra']);
							if (queueData['error']) TaskQueue.handleError(task, queueData['error'], queueData['msg']);
							if (queueData['restored'] && wman.getById('premium')) {
								west.window.premium.open();
							}
						}
					});
				TaskQueue.init(data.queue, TaskQueue.limit);
			});
		};
	}
	...
	Dobby.getAllUniqueJobs = function() {
		...
		for(var i = 0 ; i < Dobby.allJobs.length;i++) {
			...
			if(Dobby.jobFilter.filterCenterJobs && !Dobby.isInMapCenter(currentJob.x, currentJob.y) ) {
				continue;
			}
		...
		let sortBy = (field, direction) => {
			return (a, b) => {
				let isDescDirection = direction == "desc";
				if(a == null && b == null) {
					return 0;
				}
				if(a == null && b != null) {
					return isDescDirection ? 1 : -1;
				}
				if(a != null && b == null) {
					return isDescDirection ? -1 : 1;
				}
		...
		if(Dobby.jobFilter.sortField !== "" && Dobby.jobFilter.sortDirection !== ""){
			jobs.sort(sortBy(Dobby.jobFilter.sortField, Dobby.jobFilter.sortDirection))
		}
	...
	Dobby.consume = function(consumable){
		if(!Dobby.canUseConsume(consumable)){
			return false;
		}
		Dobby.removeConsumable(consumable);
		Dobby.consumableUsed.push(consumable);
		ItemUse.doIt(consumable.id);
		return true;
	};

	Dobby.tryConsume = async function(consumable, waitForConsumableCooldown) {
		//show cooldown populates Bufflist
		let itemInBag = Bag.getItemByItemId(consumable.id);
		itemInBag.showCooldown();
		if(Dobby.healthSet != -1  && consumable.health > 0) {
			Dobby.equipSet(Dobby.healthSet);
			await new Promise(r => setTimeout(r, Dobby.settings.setWearDelay*1000));
		}
		let isConsumed = false;
		if(!waitForConsumableCooldown){
			isConsumed = Dobby.consume(consumable);
		}else{
			Dobby.currentState = STATE_WAITING_CONSUMABLE;
	...
	Dobby.findProperConsumable = function(motivationMissing,energyMissing,healthMissing,averageMotivationMissing,consumables) {
		...
		if(consumables.length  == 0) return undefined;
	...
	Dobby.useConsumable = function(result, waitForConsumableCooldown) {
		let healthMissing = 100 - (Character.health/Character.maxHealth) * 100;
		let energyMissing = 100 - (Character.energy/Character.maxEnergy) * 100;
		let motivationMissing = Dobby.jobsBelowMotivation(result);
		let averageMotivationMissing = Dobby.averageMissingMotivation(result);

		let selectedConsumables = Dobby.allConsumables.filter(c => c.selected);
		let consumableToUse;
		if(waitForConsumableCooldown){
			consumableToUse = Dobby.findProperConsumable(motivationMissing, energyMissing, healthMissing, averageMotivationMissing, selectedConsumables);
		}else {
			selectedConsumables.sort((a,b) => { return b.energy - a.energy});
			consumableToUse = selectedConsumables.find(c => c.energy <= energyMissing && c.health == 0 && c.motivation == 0);
		}
		if(consumableToUse === undefined) return false;
		Dobby.tryConsume(consumableToUse, waitForConsumableCooldown);
		return true;
	};
	...
	Dobby.shouldDepositMoney = function() {
		if(Character.homeTown.town_id === 0){
			return false;
		}
		return Character.money >= Dobby.settings.depositAt ;
	}
	...
	Dobby.canAddMissing = function(result) {
		if(!Dobby.settings.addMotivation && Dobby.jobsBelowMotivation(result) && !Dobby.isStopMotivationZero()) {
			return [false, "Can't continue because of motivation"]
		}
		if(!Dobby.settings.addEnergy && Character.energy == 0) {
			return [false, "Can't continue because of energy"]
		}
		if(!Dobby.settings.addHealth && Dobby.isHealthBelowLimit()) {
			return [false, "Can't continue because of health"]
		}
		return [true, ""];
	};
	...
	Dobby.walkTo = async function(id, type, x, y) {
		await Dobby.equipSet(Dobby.travelSet);
		Guidepost.start_walk(id, type);
		//Check if reached destination
		while(true) {
			if(Map.calcWayTime(Character.position, {x: x, y: y}) == 0) {
				break;
			}
	...
	Dobby.depositMoney = async function() {
		if(Character.homeTown.town_id === 0){
			return Dobby.stop();
		}
		Dobby.currentState = STATE_DEPOSITING_MONEY;
		await Dobby.walkTo(Character.homeTown.town_id, "town", Character.homeTown.x, Character.homeTown.y);

		await BankWindow.open(Character.homeTown.town_id);
		await new Promise(r => setTimeout(r, Dobby.generateRandomNumber(5, 10) * 1000));
		$(".deposit").click();
		BankWindow.Balance.add();
		BankWindow.window.destroy();
		Dobby.run();
	};
	...
	Dobby.sleep = async function() {
		...
		Dobby.currentState = STATE_SLEEPING;
		await Dobby.equipSet(Dobby.travelSet);
		if(Dobby.settings.fortId !== -1){
			TaskQueue.add(new TaskFortSleep(Dobby.settings.fortId))
		}else {
		...
		while(true) {
			if(Character.energy === Character.maxEnergy && !Dobby.isHealthBelowLimit()) {
				break;
			}
			if(!Dobby.isRunning){
				break;
			}
			await new Promise(r => setTimeout(r, 1 * 60 * 1000)); //wait 1 min
		}
		Dobby.cancelJobs();
		Dobby.run();
	}
	...
	Dobby.tryUseConsumable = function(result){
		const [canUseConsumable, reasonNotToUse] = Dobby.canAddMissing(result);
		if(!canUseConsumable){
			if(reasonNotToUse.includes("motivation") || !Dobby.settings.sleep){
				alert(reasonNotToUse);
				Dobby.stop();
			} else{
				Dobby.sleep();
			}
		} else{
			let hasFoundConsumableToUse = Dobby.useConsumable(result, true);
			if(!hasFoundConsumableToUse){
				Dobby.stop();
			}
		}
	}
	...
	Dobby.run = function() {
		...
		Dobby.checkMotivation(0,[],function(result) {
			let shouldDepositMoney = Dobby.shouldDepositMoney();
			if((Dobby.isMotivationAbove(result) || Dobby.isStopMotivationZero()) 
				&& Character.energy > 0 
				&& !Dobby.isHealthBelowLimit()
				&& !shouldDepositMoney
			){
				Dobby.currentState = STATE_RUNNING;
				Dobby.selectTab("choosenJobs");
				if(Dobby.settings.addEnergy) Dobby.useConsumable(result, false);
				Dobby.prepareJobRun(Dobby.currentJob.job);
			}else if(shouldDepositMoney){
				Dobby.depositMoney();
			}
			else {
				Dobby.tryUseConsumable(result);
			}
		});
	};
	...
	Dobby.jobsInSameLocation = (job) => {
		if(job === undefined) return [];
		return Dobby.allJobs.filter((j) => job.x == j.x && job.y == j.y);
	};

	Dobby.getJobCanDoFromList = (jobs) => {
		return jobs.find((j) => {
			return JobList.getJobById(j.id).canDo()
		})
	};

	Dobby.walkToJob = async function(index) {
		let jobToQueue = Dobby.getJobCanDoFromList(Dobby.jobsInSameLocation(Dobby.addedJobs[index]));
		JobWindow.startJob(jobToQueue.id, jobToQueue.x, jobToQueue.y, 15);
		while(true) {
			if(Map.calcWayTime(Character.position,{x:Dobby.addedJobs[index].x,y:Dobby.addedJobs[index].y}) == 0) {
				break;
			}
	...
	Dobby.runJob = async function(jobIndex,jobCount) {
		...
		Dobby.cancelJobs();
		Dobby.statistics.jobsInSession -= queueLength;
		Dobby.statistics.totalJobs -= queueLength;
		Dobby.updateStatistics(oldXp);
		Dobby.save();
		Dobby.run();
	};
	...
	Dobby.save = function() {
		...
		localStorage.setItem("dobby2permanent", JSON.stringify(pernamentObject))
		...
		localStorage.setItem("dobby2temporary", JSON.stringify(temporaryObject))
	};
	Dobby.load = function() {
		let pernamentObject = localStorage.getItem("dobby2permanent");
		if(pernamentObject !== null){
			pernamentObject = JSON.parse(pernamentObject);
		...
		let temporaryObject = localStorage.getItem("dobby2temporary");
		if(temporaryObject !== null){
			temporaryObject = JSON.parse(temporaryObject);
			let tempObjectDate =  Date.parse(temporaryObject.date);
			let silverResetDate = new Date().setHours(SILVER_RESET_HOUR, 0, 0);
			if(tempObjectDate < silverResetDate){
				localStorage.removeItem("dobby2temporary");
				return;
			}
			let addedJobs = temporaryObject.addedJobs;
			for(var j = 0 ; j < addedJobs.length;j++) {
				var jobP = new JobPrototype(addedJobs[j].x,addedJobs[j].y,addedJobs[j].id);
				jobP.setSilver(addedJobs[j].silver);
				jobP.distance = addedJobs[j].distance;
				jobP.setExperience(addedJobs[j].experience);
				jobP.setMoney(addedJobs[j].money);
				jobP.setMotivation(addedJobs[j].motivation);
				jobP.setStopMotivation(addedJobs[j].stopMotivation);
				jobP.setSet(addedJobs[j].set);
				Dobby.addedJobs.push(jobP);
			}
			Dobby.travelSet = temporaryObject.travelSet;
			Dobby.jobSet = temporaryObject.jobSet;
			Dobby.healthSet = temporaryObject.healthSet;
			Dobby.currentJob = temporaryObject.currentJob;
	...
	Dobby.addEventsHeader = function() {
		let sortField = (field) => {
			if(Dobby.jobFilter.sortField == field){
				Dobby.jobFilter.sortField = Dobby.jobFilter.sortDirection == "asc" ? "" : field
				Dobby.jobFilter.sortDirection = Dobby.jobFilter.sortDirection == "desc" ? "asc" : "";
			}else{
				Dobby.jobFilter.sortField = field;
				Dobby.jobFilter.sortDirection = "desc";
			}
		}
	...
	Dobby.createJobsTab = function() {
		...
		let addSortArrow = (field) => {
			if(Dobby.jobFilter.sortField == field){
				return Dobby.jobFilter.sortDirection == "asc" ? arrow_asc : arrow_desc;
			}
			return "";
		}
		table
		...
		table
			...
			.appendToCell("head","jobXp",xpIcon + addSortArrow("experience"))
			.appendToCell("head","jobMoney",dollarIcon + addSortArrow("money"))
			.appendToCell("head","jobMotivation",motivationIcon)
			.appendToCell("head","jobDistance","Distance "  + addSortArrow("distance"))
	...
	Dobby.start = function() {
		if(Dobby.isRunning){
			new UserMessage("Dobby is already running", UserMessage.TYPE_ERROR).show();
			return;
		}
		if(Dobby.addedJobs.length === 0){
			new UserMessage("No jobs selected ...", UserMessage.TYPE_ERROR).show();
			return;
		}
	...
	Dobby.createSettingsGui = function() {
		...
		var checkboxSleep = new west.gui.Checkbox();
		checkboxSleep.setLabel("Sleep");
		checkboxSleep.setSelected(Dobby.settings.sleep);
		checkboxSleep.setCallback(function() {
			Dobby.settings.sleep = !Dobby.settings.sleep;
		});
		var htmlFortId = $("<div></div>");
		htmlFortId.append("<span> Fort Id </span>");
		var fortIdTextField = new west.gui.Textfield("healthStop");
		fortIdTextField.setValue(Dobby.settings.fortId);
		fortIdTextField.setWidth(100);
		htmlFortId.append(fortIdTextField.getMainDiv());
		...
		var htmlMoneyDepositAt = $("<div></div>");
		htmlMoneyDepositAt.append("<span> Deposit when money > than </span>");
		var moneyDepositAt = new west.gui.Textfield("moneyStop");
		moneyDepositAt.setValue(Dobby.settings.depositAt);
		moneyDepositAt.setWidth(100);
		htmlMoneyDepositAt.append(moneyDepositAt.getMainDiv());
		...
		var buttonApply = new west.gui.Button("Apply",function() {
			...
			Dobby.settings.sleep = checkboxSleep.isSelected();
			if(Dobby.isNumber(fortIdTextField.getValue())) {
				Dobby.settings.fortId = parseInt(fortIdTextField.getValue());
			}
			...
			if(Dobby.isNumber(moneyDepositAt.getValue())) {
				let depositAt = parseInt(moneyDepositAt.getValue());
				depositAt = Math.max(1000, depositAt);
				Dobby.settings.depositAt = depositAt;
			}
		...
		htmlSkel.append("<br>");
		htmlSkel.append(checkboxSleep.getMainDiv());
		htmlSkel.append("<br>");
		htmlSkel.append(htmlFortId);
		htmlSkel.append("<br>");
		htmlSkel.append(htmlHealthStop);
		htmlSkel.append("<br>");
		htmlSkel.append(htmlMoneyDepositAt);
		htmlSkel.append("<br>");
	...
	const createMenuIcon = function() {
		...
		var link = $('<div id="Menu" class="menulink" onclick=window.Game.Dobby.loadJobs(); title="Dobby 2" />').css('background-image', 'url(' + menuimage + ')');
	...
	$(document).ready(function() {
		try{
			...
			Dobby.loadSets(function(){});
			Dobby.load();
			Dobby.redefineFunctions();
			window.Game.Dobby = Dobby;
			createMenuIcon();
		}catch(e) {
